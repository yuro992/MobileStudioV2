...
public class ModeActivity extends Activity {
    ...
    private void buildStudioScreen() {
        ...
        studioPreviewView = new TextureView(this); studioPreviewView.setBackgroundColor(Color.BLACK); studioPreviewView.setOpaque(true); studioPreviewView.setSurfaceTextureListener(new TextureView.SurfaceTextureListener() { @Override public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) { studioPreviewTexture = surface; if (decoderOutputSurface != null) { try { decoderOutputSurface.release(); } catch (Exception ignored) { } } decoderOutputSurface = new Surface(surface); setStatus("Preview surface ready. Connect to Sender", true); updateMetrics(); } @Override public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) { studioPreviewTexture = surface; if (decoderOutputSurface == null || !decoderOutputSurface.isValid()) { decoderOutputSurface = new Surface(surface); } updateMetrics(); } @Override public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) { studioPreviewTexture = null; if (decoderOutputSurface != null) { try { decoderOutputSurface.release(); } catch (Exception ignored) { } decoderOutputSurface = null; } releaseStudioDecoder(); if (studioClientRunning) { setStatus("Preview surface lost. Disconnect and reconnect", false); } updateMetrics(); return true; } @Override public void onSurfaceTextureUpdated(SurfaceTexture surface) { } }); root.addView(studioPreviewView, fullWidthHeightWithBottom(dp(220), dp(14)));
